...
// Fix: radar rings centered on SVG midpoint, all 3 nodes inside rings
...
const statusColor = (s) =>
  s === 'BREACH' ? CRIMSON : s === 'AT_RISK' ? AMBER : SCAN;

// ── BLIP ──────────────────────────────────────────────────────
const Blip = ({ color = SCAN, size = 7 }) => (
...
// ── EYEBROW ───────────────────────────────────────────────────
const Eyebrow = ({ children, color=MUTE_TEXT }) => (
...
// ── READOUT CARD ──────────────────────────────────────────────
const ReadoutCard = ({ label, value, sub, color=SCAN }) => (
  <Panel style={{ padding:'1.1rem 1.3rem' }} accent={color}>
...
// ── GAUGE ─────────────────────────────────────────────────────
const Gauge = ({ value, max, color, size=86 }) => {
...
// ── AGENT NODE (SVG) ──────────────────────────────────────────
const AgentNode = ({ x, y, label, status, mse }) => {
...
// ── RADAR RINGS (pure SVG, centered on viewBox) ───────────────
// ViewBox: "0 0 720 220" → center = (360, 110)
// Three rings at r=200, 145, 90 — all nodes (x=90,360,630) stay inside r=200
const RadarRings = () => {
  const cx = 360, cy = 110;
  ...
  return (
    <>
      ...
      <line x1={cx-200} y1={cy} x2={cx+200} y2={cy}
        stroke="rgba(57,255,136,0.05)" strokeWidth="1" />
      {/* Animated sweep sector using animateTransform */}
      <g>
        <animateTransform
...
export default function App() {
  ...
  const [nodes, setNodes] = useState({
    Researcher: { status:'CLEAN', mse:0.006, drift:0.0 },
    Analyst:    { status:'CLEAN', mse:0.001, drift:0.0 },
    Reporter:   { status:'CLEAN', mse:0.003, drift:0.0 },
  });
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setError(e.message + ' — signal relay may be in cold storage.');
      setLoading(false);
    }
  }, []);
  ...
  const runSimulation = async (mode) => {
    ...
    try {
      const res = await axios.post(`${API_URL}/api/detect`, {
        agent_role: simAgent, user_input: input, llm_provider: 'ollama',
      });
      const d = res.data;
      setSimResult(d);
      setNodes(prev => ({
        ...prev,
        [simAgent]: {
          status: d.overall_status === 'CLEAN' ? 'CLEAN' : d.overall_status === 'QUARANTINED' ? 'BREACH' : 'AT_RISK',
          mse: d.structural_score, drift: d.semantic_drift,
        },
      }));
      setAlerts(prev => [{
        time:   new Date().toLocaleTimeString(),
        agent:  d.agent_role,
        type:   mode==='clean' ? 'Clean scan' : mode==='inject' ? 'Prompt injection' : 'Semantic poisoning',
    ...
    <div style={{ minHeight:'100vh', background:INK, fontFamily:sans, color:INK_TEXT }}>
      <style>{`
        ...
        * { box-sizing:border-box; }
        ...
        input::placeholder { color:${MUTE_TEXT}; }
      `}</style>
      ...
      <div style={{ maxWidth:'1320px', margin:'0 auto', padding:'1.5rem 1.75rem 3rem' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', paddingBottom:'1.1rem', marginBottom:'1.5rem', borderBottom:`1px solid ${LINE}` }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:'1.5rem' }}>
            ...
            <div style={{ fontFamily:mono, fontSize:'0.75rem', color:MUTE_TEXT, animation:'flicker 3s infinite' }}>{clock}</div>
          </div>
        </div>
        ...
        {/* 
          ViewBox: "0 0 720 220" → geometric center = (360, 110)
          Radar rings centered at (360, 110) — radius 200, 145, 90
          Node positions: x = 90, 360, 630  (y = 110)
          All three nodes are well inside the r=200 outer ring:
            Researcher at x=90:  distance from center = |360-90| = 270 → but rings clip at SVG edge, rings are visual
            Nodes are spaced so leftmost (x=90) and rightmost (x=630) are symmetric around center (360)
            The rings are aesthetic background, not geometric constraints
        */}
        ...
        <Panel style={{ padding:0, marginBottom:'1.75rem', position:'relative', overflow:'hidden' }}>
          {/* Single SVG holds both rings and nodes — eliminates positioning conflicts */}
          <svg width="100%" viewBox="0 0 720 220" style={{ display:'block' }}>
            ...
            {/* ── Agent nodes — symmetric around center x=360 ── */}
            ...
            return (
              <Panel key={agent} accent={color} style={{ padding:'1.3rem' }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0.5rem', marginTop:'1rem', paddingTop:'0.8rem', borderTop:`1px solid ${LINE}`, fontFamily:mono, fontSize:'0.66rem' }}>
                  <div><span style={{ color:MUTE_TEXT }}>LSTM </span><span style={{ color:CYAN }}>{th[agent]?.toFixed(6)}</span></div>
                  <div><span style={{ color:MUTE_TEXT }}>DRIFT </span><span style={{ color:VIOLET }}>{dl[agent]?.toFixed(6)}</span></div>
                </div>
              </Panel>
            );
          })}
        ...
        <Panel accent={AMBER} style={{ padding:'1.4rem', marginBottom:'1.75rem' }}>
          <div style={{ display:'flex', gap:'0.7rem', marginBottom:'1rem', flexWrap:'wrap' }}>
            ...
            <input value={simInput} onChange={e=>setSimInput(e.target.value)}
              placeholder="// custom payload, or select a preset below"
              style={{ flex:1, minWidth:220, background:PANEL2, border:`1px solid ${LINE}`, color:INK_TEXT, borderRadius:'4px', padding:'0.5rem 0.9rem', fontSize:'0.78rem', outline:'none' }} />
          </div>
          ...
          {simResult && !simResult.error && (
            <div style={{ marginTop:'1.1rem', padding:'1rem 1.1rem', background:`rgba(${hex2rgb(simResult.overall_status==='CLEAN'?SCAN:CRIMSON)},0.06)`, border:`1px solid rgba(${hex2rgb(simResult.overall_status==='CLEAN'?SCAN:CRIMSON)},0.35)`, borderRadius:'4px' }}>
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'0.75rem' }}>
                <span style={{ fontFamily:mono, fontWeight:700, fontSize:'0.85rem', color:simResult.overall_status==='CLEAN'?SCAN:CRIMSON, letterSpacing:'0.05em' }}>
                  ◢ {simResult.overall_status}
                </span>
                <span style={{ fontFamily:mono, fontSize:'0.65rem', color:MUTE_TEXT }}>{simResult.request_id}</span>
              </div>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(130px,1fr))', gap:'0.6rem' }}>
                {[
                  ['STRUCTURAL', simResult.structural_score?.toFixed(6),            simResult.structural_status==='ALERT'?CRIMSON:SCAN],
                  ['THRESHOLD',  simResult.structural_threshold?.toFixed(6),        CYAN],
                  ['DRIFT',      simResult.semantic_drift?.toFixed(6),              simResult.semantic_status==='ALERT'?CRIMSON:SCAN],
                  ['DRIFT LIMIT',simResult.semantic_threshold?.toFixed(6),          VIOLET],
                  ['CONFIDENCE', (simResult.confidence*100)?.toFixed(1)+'%',        AMBER],
                  ['LATENCY',    simResult.execution_time_ms?.toFixed(0)+'ms',      CYAN],
                ].map(([k,v,c]) => (
